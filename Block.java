...
/**
 * Abstract class to model Sudoku block. Inherited by BlockNumeic and BlockTextual.
 */
...
/**
 * Super constructor for abstract Block class. Called explicitly by derived classes to initialise fields
 ...
 */
    public Block(int size, Scanner reader){
        this.reader = reader; //Assigns Scanner object passed as parameter to object field. Easier to share reference to single Scanner object between objects than to close current Scanner and then instantiate new Scanner object.
        this.size = size;
        ...
        //Overwrite default null elements in rows and columns arrays to new instances of Line, representing currently blank lines in the block
    ...
    /**
     * Called from main to initialise constructed Block object. Uses CheckFormat and CheckStructure methods to populate and validate block with user-input
     */
    public void initialise(){
        System.out.println("Enter cell values:"); 
        this.checkFormat();
        if (!this.formatValid){System.out.println(this.errorDescription);}
        else{
            this.checkStructure();
            if (!this.structureValid){System.out.println(this.errorDescription);}
            else{System.out.println("Valid format, valid structure.");}
        }
    }
    ...
    /**
     * Called from Initialise in Block super class. Reads user input of block element values. 
     * Validates block format by checking:
     *  -Number of cells input for row
     *  -Input value of cells
     *  -Number of rows input
     * Records results of checks to boolean flag formatValid, writes error output to errorDescription String
     */
    ...
    private void checkStructure(){
        ...
        Line blockLine=this.getNewValueLine();

        //Construct and check rows for repeats, construct columns
        for (int i = 0; i < this.size; i++) { //Iterating for each row
            this.rows[i].addCells(this.cells[i]); //Adding all cells in row's values to row's bit mask
            if (!this.rows[i].getIsValid()){ //If row contains repeated values
                this.structureValid=false; 
                this.errorDescription="Valid format, invalid structure: "+ this.rows[i].getFirstDuplicate()+ " repeated in row " +(i+1)+".\n"; //In the case of multiple duplicate values, only the first is displayed by accessing index 0 of row duplicatedValues list
                return; //Do not check for further structure errors
            }
            
            //For each row, add cell values to respective columns
            for (int j = 0; j < this.cells[i].length; j++) {
                this.columns[j].addCell(this.cells[i][j]); //Adding single cell value to column's bit mask
            }


            blockLine.addCells(this.cells[i]);
        ...
        for (int i = 0; i < this.columns.length; i++) { //Iterating for each column
            if (!this.columns[i].getIsValid()){ //If column contains repeated values
                this.structureValid=false;
                this.errorDescription="Valid format, invalid structure: "+this.columns[i].getFirstDuplicate()+" repeated in column "+(i+1)+".\n"; //In the case of multiple duplicate values, only the first is displayed by accessing index 0 of row duplicatedValues list
                return; //Do not check for further structure errors
            }
        }

        if (!blockLine.getIsValid()){
            this.structureValid=false;
            this.errorDescription="Valid format, invalid structure: "+blockLine.getFirstDuplicate()+" repeated in block.\n";
            return;
        }

        /*
        //Check whole block
        Line valueLine = this.getNewValueLine(); //Creating a new line with mask to contain all values in block. Value in link mask added to row by row, can be thought of as running account of values in cells in rows so far
        for (Line row : this.rows) { //Iterating for each row in block
            if (valueLine.hasValuesInCommon(row)){ //Check if row contains values already encountered in block
                this.structureValid=false;
                this.errorDescription="Valid format, invalid structure: "+valueLine.getFirstValueInCommon(row)+" repeated in block.\n"; //In the case of multiple duplicate values, only the first is displayed
                return; //Do not check for further structure errors
            }
            else{
                valueLine.mergeBitFields(row); //If no duplicate values in row, then add the values of the row to running account of block values by merging value masks
            }
        }
        */
